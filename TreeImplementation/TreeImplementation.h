...
struct Tree
{
    vector <Tree<treeType>*> childrenV;
    treeType nodeData;
};
...
template <class treeType>
Tree<treeType>* InitTree(int levelNumber, int childIndex, treeType parentData)
{
    Tree<treeType>* node = new Tree<treeType>;
    cout << "Enter the data for node at level: " << levelNumber << " child index: " << childIndex << ": ";
    cin >> node->nodeData;
    cout << endl;
    int newNodes;
    cout << "How many new nodes at level: " << levelNumber << " child index: " << childIndex << "? ";
    cin >> newNodes;
    cout << endl;
    for (int ix = 1; ix <= newNodes; ix++)
    {
        auto child = InitTree<treeType>(levelNumber + 1, ix, node->nodeData);

        node->childrenV.push_back(child);
    }
    return node;
}

template <class treeType>
void DumpTree(Tree<treeType>* rootNode, int levelIndex, int childIndex, treeType parentData)
{
    cout << "Node Data at level: " << levelIndex << " child index: " << childIndex << ": " << " of parent:" << parentData << ": " << rootNode->nodeData << endl;
    for (auto ix = 0U; ix < rootNode->childrenV.size(); ix++)
    {
        DumpTree(rootNode->childrenV[ix], levelIndex + 1, ix + 1, rootNode->nodeData);
    }
}
...
template <class treeType>
int countNodes(BinTree<treeType>* root)
{
    ...
    int leftDepth = countNodes(root->lChild);
    int rightDepth = countNodes(root->rChild);
